...
function WorksForm() {
    ...
    const validate = useCallback((data) => {
        const errors = {};
        try { validatorAlphaNumeric((data.jobTitle?.[primaryLang]), "FALTA TEXTO ERROR ALPHA NUMERIC") } catch (error) { errors.jobTitlePrimary = error.message; };
        if (showOtherLang) { try { validatorAlphaNumeric((data.jobTitle?.[secondaryLang]), "FALTA TEXTO ERROR ALPHA NUMERIC") } catch (error) { errors.jobTitleSecondary = error.message; } };
        ...
        try { validatorURL(data.linkCompany), "FALTA TEXTO ERROR URL COMPANY" } catch (error) { errors.linkCompany = error.message; };
        ...
        if(showOtherLang) { try { validatorLongText(data.description?.[secondaryLang]), "FALTA TEXTO ERROR LONG TEXT"} catch (error) { errors.descriptionSecondary = error.message; }};
        return errors;
    ...
    useEffect(() => {
        const loadWork = async () => {
            try {
                if(!user) return;
                const permission = isEdit ? "update_work": "create_work";
            ...
            <section className="workFormSectForm">
                <form id="workForm" onSubmit={handleSubmit}>
                    <div className="workFormCheckCont">
                        ...
                        {isEdit && (
                            <Inputs textH2={TEXT.ID} type="text" name="_id" value={formData._id} language={language} readOnly={true} disabled={true} className={""}
